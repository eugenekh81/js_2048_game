...
function hasMoves(array) {
    for(let r = 0; r < NUMBER_OF_ROWS; r++)for(let c = 0; c < NUMBER_OF_CELLS - 1; c++){
...
function moveTiles(e) {
    ...
    if (moved) generateRandomTiles(1);
    boardValues = getFieldValues();
...
function checkIfEqual(original, updated) {
    ...
    for(let r = 0; r < NUMBER_OF_ROWS; r++)for(let c = 0; c < NUMBER_OF_CELLS; c++){
...
function slideLeft(array) {
    ...
    for(let r = 0; r < NUMBER_OF_ROWS; r++){
        ...
        for(let c = 0; c < NUMBER_OF_CELLS - 1; c++)if (row[c] && row[c] === row[c + 1]) {
            row[c] *= 2;
            row[c + 1] = 0;
...
function rotateArray(array) {
    const rotadedArray = [];
    for(let c = 0; c < NUMBER_OF_CELLS; c++)for(let r = 0; r < NUMBER_OF_ROWS; r++){
...
function addZeroes(row) {
    for(let c = 0; c < NUMBER_OF_CELLS; c++)if (!row[c]) row.push(0);
    return row;
}
...
function isGameWon() {
    return [
        ...refs.tiles
    ].some((tile)=>+tile.textContent === WIN_VALUE);
}
